...
The station callsign is fixed in the PIC16F84A firmware. It is **not field
programmable**, and neither rotary switch changes it. The callsign is separate
from the fox message selected by S1. Confirm that each controller contains a PIC
programmed with the intended callsign before deployment.
...
Powering all five at the same time would put their normal cycles at
approximately the same phase and cause continuing overlap.
